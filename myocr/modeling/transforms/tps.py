...
class LocalizationNetwork(nn.Module):
    def __init__(self, in_channels, num_fiducial, loc_lr, model_name):
        ...
        for fno in range(0, len(num_filters_list)):
            ...
            conv = ConvBNLayer(
                in_channels=in_channels, out_channels=num_filters, kernel_size=3, act="relu"
            )
...
class GridGenerator(nn.Module):
    ...
    def forward(self, batch_C_prime, I_r_size):
        ...
        batch_T = torch.matmul(
            inv_delta_C_tensor.to(batch_C_prime_with_zeros.device), batch_C_prime_with_zeros
        )
    ...
    def build_inv_delta_C_paddle(self, C):
        ...
        delta_C = torch.cat(  # F+3 x F+3
            (
                ...
                torch.concat((torch.zeros((2, 3)), C.transpose(0, 1)), dim=1),  # 2 x F+3
                torch.concat((torch.zeros((1, 3)), torch.ones((1, F))), dim=1),  # 1 x F+3
            ),
            dim=0,
        )
...
class TPS(nn.Module):
    ...
    def forward(self, image):
        ...
        if is_fp16:
            batch_I_r = batch_I_r.astype(data_type)  # type: ignore
