...
from myocr.types import DetectedObjects, RecognizedTexts
from myocr.utils import LabelTranslator

from .base import LabelDecoder, PadNdarray, RotateImg
...
class TextRecognitionProcessor(CompositeProcessor[DetectedObjects, RecognizedTexts]):
    def __init__(self):
        super().__init__()
    ...
    def preprocess(self, input_data: DetectedObjects) -> Optional[np.ndarray]:
        ...
        for box in input_data.bounding_boxes:  # type: ignore
            ...
            # 中心旋转后在填充
            if box.angle[0] > 0:
                resized_img = RotateImg(box.angle[0]).process(resized_img)
    ...
    def postprocess(self, internal_result: np.ndarray) -> Optional[RecognizedTexts]:
        result = RecognizedTexts()
        texts = LabelDecoder(self.translator, self.bounding_boxes).process(internal_result)
        for t in texts:
            result.add_text(text=t[0], confidence=t[1], bounding_box=t[2])
        return result
